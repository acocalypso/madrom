...
static char *tu_tu_stacfgpath = "/vendor/etc/wifi/ssv6x5x/ssv6x5x-wifi.cfg";
EXPORT_SYMBOL(tu_tu_stacfgpath);
module_param(tu_tu_stacfgpath, charp, 0000);
MODULE_PARM_DESC(tu_tu_stacfgpath, "Get path of sta cfg");
char *tu_tu_cfgfirmwarepath = NULL;
EXPORT_SYMBOL(tu_tu_cfgfirmwarepath);
module_param(tu_tu_cfgfirmwarepath, charp, 0000);
MODULE_PARM_DESC(tu_tu_cfgfirmwarepath, "Get firmware path");
char* tu_tu_ssv_initmac = NULL;
EXPORT_SYMBOL(tu_tu_ssv_initmac);
module_param(tu_tu_ssv_initmac, charp, 0644);
MODULE_PARM_DESC(tu_tu_ssv_initmac, "Wi-Fi MAC address");
...
extern struct ssv6xxx_cfg_cmd_table tu_tu_cfg_cmds[];
...
static void _set_initial_cfg_default(void)
{
  ...
  if ((tu_tu_cfg_cmds[s].def_val)!= NULL) {
   tu_tu_cfg_cmds[s].translate_func(tu_tu_cfg_cmds[s].def_val,
    tu_tu_cfg_cmds[s].var, tu_tu_cfg_cmds[s].arg);
  }
 }
}
static void _import_default_cfg (char *tu_tu_stacfgpath)
{
 struct file *fp = (struct file *) NULL;
 char buf[MAX_CHARS_PER_LINE], cfg_cmd[32], cfg_value[32];
 mm_segment_t fs;
 size_t s, read_len = 0, is_cmd_support = 0;
 printk("\n*** %s, %s ***\n\n", __func__, tu_tu_stacfgpath);
 if (tu_tu_stacfgpath == NULL)
  return;
 memset(&tu_tu_ssv_cfg, 0, sizeof(tu_tu_ssv_cfg));
 memset(buf, 0, sizeof(buf));
 _set_initial_cfg_default();
 fp = filp_open(tu_tu_stacfgpath, O_RDONLY, 0);
 ...
 do {
  ...
  is_cmd_support = 0;
  for(s=0; tu_tu_cfg_cmds[s].cfg_cmd != NULL; s++) {
   if (strcmp(tu_tu_cfg_cmds[s].cfg_cmd, cfg_cmd)==0) {
    tu_tu_cfg_cmds[s].translate_func(cfg_value,
     tu_tu_cfg_cmds[s].var, tu_tu_cfg_cmds[s].arg);
    is_cmd_support = 1;
    break;
   }
...
#if (defined(CONFIG_SSV_SUPPORT_ANDROID)||defined(CONFIG_SSV_BUILD_AS_ONE_KO))
int tu_tu_ssvdevice_init(void)
#else
static int __init tu_tu_ssvdevice_init(void)
#endif
{
    _import_default_cfg(tu_tu_stacfgpath);
    ...
    {
        int ret;
        ret = tu_tu_ssv6xxx_hci_init();
        if(!ret){
            ret = tu_tu_ssv6xxx_init();
        }if(!ret){
            ret = tu_tu_ssv6xxx_sdio_init();
...
#if (defined(CONFIG_SSV_SUPPORT_ANDROID)||defined(CONFIG_SSV_BUILD_AS_ONE_KO))
void tu_tu_ssvdevice_exit(void)
#else
static void __exit tu_tu_ssvdevice_exit(void)
#endif
{
#if (defined(CONFIG_SSV_SUPPORT_ANDROID)||defined(CONFIG_SSV_BUILD_AS_ONE_KO))
    tu_tu_ssv6xxx_exit();
    tu_tu_ssv6xxx_hci_exit();
    tu_tu_ssv6xxx_sdio_exit();
#if (defined(SSV_SUPPORT_SSV6006))
    ssv6xxx_usb_exit();
#endif
#endif
...
EXPORT_SYMBOL(tu_tu_ssvdevice_init);
EXPORT_SYMBOL(tu_tu_ssvdevice_exit);
#else
module_init(tu_tu_ssvdevice_init);
module_exit(tu_tu_ssvdevice_exit);
#endif
