...
extern struct ssv6xxx_cfg tu_tu_ssv_cfg;
...
static int tu_tu_ssv6xxx_init_softc(struct ssv_softc *sc)
{
    void *channels;
    int ret=0;
#ifdef MULTI_THREAD_ENCRYPT
...
#endif
extern char *tu_tu_cfgfirmwarepath ;
int ssv6xxx_load_firmware(struct ssv_hw *sh)
{
    ...
    if (tu_tu_cfgfirmwarepath != NULL)
    {
        snprintf(temp_path, SSV_FIRMWARE_PATH_MAX, "%s%s", tu_tu_cfgfirmwarepath,
                 firmware_name);
...
int tu_tu_ssv6xxx_init_mac(struct ssv_hw *sh)
{
...
#ifdef SSV_SUPPORT_HAL
static int tu_tu_ssv6xxx_init_hw(struct ssv_hw *sh)
{
    int ret = 0;
...
#endif
    while(tu_tu_ssv_cfg.configuration[x][0])
    {
        for(i=0; i<sizeof(ssv6200_rf_tbl)/sizeof(ssv_cabrio_reg); i++)
        {
            if(ssv6200_rf_tbl[i].address == tu_tu_ssv_cfg.configuration[x][0])
            {
                ssv6200_rf_tbl[i].data = tu_tu_ssv_cfg.configuration[x][1];
                break;
            }
        }
        for(i=0; i<sizeof(phy_setting)/sizeof(ssv_cabrio_reg); i++)
        {
            if(phy_setting[i].address == tu_tu_ssv_cfg.configuration[x][0])
            {
                phy_setting[i].data = tu_tu_ssv_cfg.configuration[x][1];
                break;
            }
        }
        x++;
...
void ssv6xxx_restart_hw(struct work_struct *work)
{
    ...
    udelay(50);
    tu_tu_ssv6xxx_init_hw(sc->sh);
    tu_tu_ssv6xxx_init_mac(sc->sh);
...
static void ssv6xxx_check_mac2(struct ssv_hw *sh)
{
  ...
  bool invalid = false;
  for ( i=0; i<6; i++) {
       if ((tu_tu_ssv_cfg.maddr[0][i] & addr_mask[i]) !=
               (tu_tu_ssv_cfg.maddr[1][i] & addr_mask[i])){
           invalid = true;
           printk (" i %d , mac1[i] %x, mac2[i] %x, mask %x \n",i, tu_tu_ssv_cfg.maddr[0][i] ,tu_tu_ssv_cfg.maddr[1][i],addr_mask[i]);
           break;
       }
   }
   if (invalid){
       memcpy(&tu_tu_ssv_cfg.maddr[1][0], &tu_tu_ssv_cfg.maddr[0][0], 6);
       tu_tu_ssv_cfg.maddr[1][5] ^= 0x01;
       if (tu_tu_ssv_cfg.maddr[1][5] < tu_tu_ssv_cfg.maddr[0][5]){
           u8 temp;
           temp = tu_tu_ssv_cfg.maddr[0][5];
           tu_tu_ssv_cfg.maddr[0][5] = tu_tu_ssv_cfg.maddr[1][5];
           tu_tu_ssv_cfg.maddr[1][5] = temp;
           sh->cfg.maddr[0][5] = tu_tu_ssv_cfg.maddr[0][5];
       }
       printk("MAC 2 address invalid!!\n" );
       printk("After modification, MAC1 %pM, MAC2 %pM\n",tu_tu_ssv_cfg.maddr[0],
           tu_tu_ssv_cfg.maddr[1]);
   }
}
static int ssv6xxx_read_configuration(struct ssv_hw *sh)
{
    memcpy(&sh->cfg, &tu_tu_ssv_cfg, sizeof(struct ssv6xxx_cfg));
    ...
    if (SSV_IF_CHK_MAC2(sh)) {
        ...
        memcpy(&sh->cfg.maddr[1][0], &tu_tu_ssv_cfg.maddr[1][0], ETH_ALEN);
    }
    #if 0
    ...
    if (!sh->flash_config.exist) {
        if(sh->cfg.volt_regulator < 2)
            sh->cfg.volt_regulator = tu_tu_ssv_cfg.volt_regulator;
        else
        {
...
#endif
static int tu_tu_ssv6xxx_init_device(struct ssv_softc *sc, const char *name)
{
...
#else
    if ((error = tu_tu_ssv6xxx_init_hal(sc)) != 0) {
#endif
        goto err;
    }
    sh = sc->sh;
    if ((error = tu_tu_ssv6xxx_hci_register(&sh->hci)) != 0)
        goto err_sh;;
    ...
    if ((error = tu_tu_ssv6xxx_init_softc(sc)) != 0) {
        goto err_softc;
    }
    ssv6xxx_set_80211_hw_capab(sc);
    if ((error = tu_tu_ssv6xxx_init_hw(sc->sh)) != 0) {
        goto err_hw;
    }
#ifndef CONFIG_SSV6XXX_HW_DEBUG
...
#endif
#ifdef CONFIG_SSV6XXX_DEBUGFS
    tu_tu_ssv6xxx_init_debugfs(sc, name);
#endif
...
err_hci:
    tu_tu_ssv6xxx_hci_deregister(&sh->hci);
err_sh:
...
static void ssv6xxx_deinit_device(struct ssv_softc *sc)
{
    ...
    ssv6xxx_deinit_hw(sc);
    ssv6xxx_deinit_softc(sc);
    tu_tu_ssv6xxx_hci_deregister(&sc->sh->hci);
    ssv6xxx_deinit_hwsh(sc);
}
extern struct ieee80211_ops ssv6200_ops;
int tu_tu_ssv6xxx_dev_probe(struct platform_device *pdev)
{
    struct ssv_softc *sc;
 ...
 if (hw == NULL) {
    ...
    ret = tu_tu_ssv6xxx_init_device(sc, pdev->name);
    if (ret) {
...
EXPORT_SYMBOL(tu_tu_ssv6xxx_dev_probe);
static void ssv6xxx_stop_all_running_threads(struct ssv_softc *sc) {
...
int tu_tu_ssv6xxx_dev_remove(struct platform_device *pdev)
{
    struct ieee80211_hw *hw=dev_get_drvdata(&pdev->dev);
    struct ssv_softc *sc=hw->priv;
    printk("tu_tu_ssv6xxx_dev_remove(): pdev=%p, hw=%p\n", pdev, hw);
...
EXPORT_SYMBOL(tu_tu_ssv6xxx_dev_remove);
...
{
    .probe = tu_tu_ssv6xxx_dev_probe,
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,7,0)
    .remove = __devexit_p(tu_tu_ssv6xxx_dev_remove),
#else
    .remove = tu_tu_ssv6xxx_dev_remove,
#endif
...
int tu_tu_ssv6xxx_init(void)
#else
static int __init tu_tu_ssv6xxx_init(void)
#endif
{
 int ret = 0;
#ifndef SSV_SUPPORT_HAL
...
#if (defined(CONFIG_SSV_SUPPORT_ANDROID)||defined(CONFIG_SSV_BUILD_AS_ONE_KO))
void tu_tu_ssv6xxx_exit(void)
#else
static void __exit tu_tu_ssv6xxx_exit(void)
#endif
{
...
#if (defined(CONFIG_SSV_SUPPORT_ANDROID)||defined(CONFIG_SSV_BUILD_AS_ONE_KO))
EXPORT_SYMBOL(tu_tu_ssv6xxx_init);
EXPORT_SYMBOL(tu_tu_ssv6xxx_exit);
#else
module_init(tu_tu_ssv6xxx_init);
module_exit(tu_tu_ssv6xxx_exit);
#endif
