...
static void tu_tu_ssv6xxx_init_ch_cfg(struct ssv_hw *sh)
{
    struct ssv_softc *sc = sh->sc;
    dbgprint(&sc->cmd_data, sc->log_ctrl, LOG_HAL,
     "Not support set channel dependant cfg for this model!! \n");
}
static void tu_tu_ssv6xxx_init_iqk(struct ssv_hw *sh)
{
    struct ssv_softc *sc = sh->sc;
    dbgprint(&sc->cmd_data, sc->log_ctrl, LOG_HAL,
...
static void tu_tu_ssv6xxx_init_gpio_cfg(struct ssv_hw *sh)
{
    struct ssv_softc *sc = sh->sc;
    dbgprint(&sc->cmd_data, sc->log_ctrl, LOG_HAL,
     "%s is not supported for this model!!\n",__func__);
...
static void ssv6xxx_attach_common_hal (struct ssv_hal_ops *hal_ops)
{
    ...
    hal_ops->init_ch_cfg = tu_tu_ssv6xxx_init_ch_cfg;
    hal_ops->init_iqk = tu_tu_ssv6xxx_init_iqk;
 ...
 hal_ops->update_rf_pwr = ssv6xxx_update_rf_pwr;
    hal_ops->init_gpio_cfg = tu_tu_ssv6xxx_init_gpio_cfg;
...
int tu_tu_ssv6xxx_init_hal(struct ssv_softc *sc)
{
    struct ssv_hw *sh;
    int ret = 0;
