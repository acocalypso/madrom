...
static void ssv6051_cabrioE_update_hw_config(struct ssv_hw *sh, ssv_cabrio_reg *rf_table, ssv_cabrio_reg *phy_table)
{
    ...
    while(tu_ssv_cfg.configuration[x][0])
    {
        ...
        {
            if(rf_table[i].address == tu_ssv_cfg.configuration[x][0])
            {
                rf_table[i].data = tu_ssv_cfg.configuration[x][1];
                break;
            }
        }
        for(i = 0; i < ssv6051_cabrioE_phy_tbl_size/sizeof(ssv_cabrio_reg); i++)
        {
            if(phy_table[i].address == tu_ssv_cfg.configuration[x][0])
            {
                phy_table[i].data = tu_ssv_cfg.configuration[x][1];
                break;
            }
        }
        x++;
